...
class SelectorTreeSitter {
  ...
  walkSelector(node, idx) {
    ...
    // IMPORTANT : It's vital to remember, that the `node` we have, is always
    // the last successfully matched node.
    // ^^^ Doesn't seem to be accurate anymore
    // The node provided will be what's needed. Based off the last matched node
    // We will use the current token as the guide in what to do with any given node
    ...
    if (token.type === "class") {
      ...
      if (this.verbose) {
        console.log("Token: pseudo-class");
      }
      ...
      if (this.verbose) {
        console.log("Token: pseudo-element");
      }
